...
- [X] T219 Playwright: Platform Admin happy path
  - **Purpose**: Follows this project's established live-browser methodology (Epics 7–9).
  - **Files**: `frontend/e2e/t219-platform-admin-happy-path.spec.ts`, `frontend/e2e/helpers.ts`, `frontend/playwright.config.ts` (new — real Playwright, not a scratchpad script; see below)
  - **Deps**: T218, T204
  ...
  - **Result (2026-08-24)**: DONE. The T219-T221 tooling gap recorded in Phase 17 is resolved: `@playwright/test` installed as a real devDependency (`frontend/package.json`), Chromium run via the official `mcr.microsoft.com/playwright` Docker image against the actual `erp-system-web-1`/`erp-system-api-1` containers over `--network host`. Real browser clicks/form-fills, not HTTP-equivalent substitutes: login → dashboard (header assertion) → Tenants search (by slug) → tenant detail (status) → Subscriptions (assign `T217 Allow Plan`) → Entitlements (grant a `crm` override, "Permanent override" badge) → Audit (filtered by company id, both `subscription.assign_or_change` and `entitlement_override.grant` events visible). Zero console errors (excluding Chromium's own automatic "Failed to load resource" logging for the suite's *intentionally*-triggered 403s in T220/T221 — never emitted by application code, so it can never indicate an app defect; see `helpers.ts`). **Two real, previously-undiscovered defects found and fixed en route** (both blocking, both outside Epic 9A's own module but load-bearing for this task): (1) `GET /api/v1/companies` (list-my-companies) had **no route at all** — 405 on every call — even though the frontend's `listCompanies()` and `CompanyService.list_user_companies()` both already existed; the two were simply never wired together. Fixed by adding the missing `GET ""` route in `companies/router.py`; 3 new regression tests added (`test_list_my_companies.py`). (2) The Platform Tenants page's search box (placeholder "Search by name or slug…") only ever matched `legal_name` — searching by slug silently returned zero results for every tenant, always. Fixed by widening `CompanyRepository.list_all()`'s search filter to match `legal_name` OR `slug`. **One additional defect found and fixed, not part of this task's own scope but blocking it**: `PlatformAuthContext.tsx`'s (and, found to share the identical pattern, `AuthContext.tsx`'s) session-hydration effect had a real React Strict Mode race — the dev-only synchronous mount→cleanup→remount double-invocation set a `cancelled` flag from the throwaway first invocation that then permanently poisoned the one genuine in-flight `hydrate()` call's eventual `setIsAuthenticated`/`setIsLoading` updates, stranding the actually-mounted component on an infinite loading spinner after every fresh login. Only real browser navigation could have surfaced this — no unit test or backend/HTTP check exercises React's own mount lifecycle. Fixed in both contexts by relying solely on the existing `hydrateStartedRef` guard (already sufficient, since it caps `hydrate()` to at most one real call) and removing the harmful `cancelled` flag; React 18+ already no-ops `setState` after a genuine unmount, so no replacement guard was needed. Full frontend regression after the fix: 121/121 Jest passing (no change in count — proves no regression), `tsc --noEmit` 0 errors, `eslint .` 0 errors (same 55 pre-existing warnings, none in any touched file). Backend regression: companies module 213+3 passing, full `platform_admin`+`companies` sweep 563 passing (+3 contract-conformance tests independently verified 3/3 via a temporary `specs/` mount — the same pre-existing container-mount limitation documented in Phase 16), and finally the **full backend suite** (`tests/`, excluding those same 2 container-mount-limited files): 5936 passed, 3 failed, 8 skipped in 81 minutes (~5939 tests) — all 3 failures characterized per the Pre-Existing Failure Policy: 2 (`test_report_performance.py` regression guard, `test_recurring_journal_service.py`) FAILED full suite + PASSED isolation (load-sensitive under a ~5900-test concurrent run); 1 (`test_settings.py::test_settings_default_debug_is_false`) is a deterministic, pre-existing environment-config characteristic — this docker-compose dev stack runs with `DEBUG=true` by design (`docker-compose.yml`), unrelated to any change here. None of the 3 touch `companies/`, `auth/`, or `platform_admin/`.

- [X] T220 Playwright: suspension flow in the browser
  - **Files**: `frontend/e2e/t220-suspension-flow.spec.ts`
  - **Deps**: T219
  ...
  - **Result (2026-08-24)**: DONE, real two-browser-context run (tenant + platform admin, same Docker stack). Tenant confirms access to Company A via a real in-page `<Link>` click (same in-memory access token throughout — `tokenStorage.ts` keeps the access token in-memory only). Admin suspends A with a typed reason via the real dialog → tenant's next in-page navigation to A is denied (`role="alert"`, "Company is suspended..."), still using the *same* in-memory token — no reload happened. Admin reactivates → tenant, still on the same token, is *still* denied (watermark holds through reactivation, exactly as required). **Critical step, now genuinely proven in a browser**: a real `page.reload()` clears the in-memory access token and forces `AuthContext`'s actual hydration path to call the real `POST /auth/refresh` using the refresh token already sitting in `localStorage` from the original login — the resulting *new* access token is still bound to the same pre-suspension Session, so the tenant is *still* denied. A genuine logout+re-login (new Session) finally restores access. One test-construction finding, not an app defect: the tenant app has no logout button anywhere in its UI (`AuthContext` exposes `logout()`, nothing renders it) — worked around by clearing the stored refresh token directly before the genuine-login step (documented inline in the spec), which also avoids a real race between silent hydration-refresh (using the still-present old token) and the explicit form login. Zero unexpected console errors.

- [X] T221 Playwright: multi-tenant and support flows
  - **Files**: `frontend/e2e/t221-multi-tenant-support-access.spec.ts`
  - **Deps**: T220
  ...
  - **Result (2026-08-24)**: DONE, real two-browser-context run. Tenant confirms access to both A and B; admin suspends A only; tenant's same session is denied on A and *still fully functional on B* (genuine multi-tenant isolation, same access token). Admin initiates a support-access grant for A (reason required) → "PRIVILEGED SUPPORT ACCESS ACTIVE" banner (administrative context visible). Business-records-unavailable proven via real Next.js routing from a second page in the same admin browser context (so the grant's session-local "active" state on the first page survives): `GET /platform-admin/tenants/{id}/inventory` and `.../sales` both hit Next.js's real not-found handling — no such route exists anywhere in the Platform Admin app, matching T218's own backend-level 404 finding, now also proven at the UI/routing layer. Terminate ends the grant (banner disappears). Zero unexpected console errors. All 3 specs (T219+T220+T221) pass together in one combined suite run, twice consecutively, confirming no leftover-state flakiness between them (each spec idempotently ensures Company A starts `active`/override-free via `helpers.ts`'s `ensureCompanyActive`/`ensureNoActiveOverride`, since T220/T221 suspend A as part of their own scenario).
...
**T219-T221 closure (2026-08-24, separate follow-up session)**: the tooling gap above is now resolved — real Playwright/Chromium installed and run against the live Docker stack; see T219/T220/T221's own updated Result lines above for full evidence. All three now genuinely pass, individually and together in one combined run (twice consecutively). Three real, previously-undiscovered defects were found and fixed in the process: a missing `GET /api/v1/companies` route, a Platform-Tenants-search-by-slug filter gap, and a React Strict Mode session-hydration race affecting both `AuthContext` and `PlatformAuthContext` (full detail in T219's Result line). None of the three defects were introduced by Epic 9A's own Phase 1-17 work — all three pre-date this closure session and were simply never exercised by any prior backend/HTTP-only verification. Epic 9A is now fully closed with **zero remaining deviations**. See the Phase 17 PHR follow-up (`history/prompts/009a-platform-admin/0032-*.prompt.md`) for full evidence.
...
**Phase 17 verification-coverage note (2026-08-23, closed 2026-08-24)**: every row above that cites T219, T220, or T221 as verification evidence (US-1, US-2, US-3, US-8, US-9, FR-9A-017/018/222) is now backed by genuine Playwright browser verification (real Chromium against the live Docker stack), in addition to the real HTTP/Postgres-level proof already recorded by T215 (US-1/US-2/US-8's underlying flows), T216 (US-3, FR-9A-017/018/222), and T218 (US-9) — see T219/T220/T221's own Result notes above for full evidence, including three real pre-existing defects the browser-level pass uniquely surfaced.
...
- [X] **Gate G** (T225, Phase 17) — real-stack regression across Epics 1–9. **PASS**, proven by T214-T218 (real Postgres/HTTP) + T222-T224 (620+972 regression tests) + T219-T221 (real Playwright/Chromium browser verification, closed 2026-08-24 — no remaining caveat).
...
- [X] All 12 user stories have implementation + verification coverage, including genuine Playwright browser verification for T219/T220/T221 (closed 2026-08-24).
...
- [X] **PostgreSQL, Docker Compose and Playwright verification all exist.** PostgreSQL and Docker Compose verification: complete (T214-T218, T222-T224). Playwright verification: complete (T219-T221, closed 2026-08-24 — real Chromium via the official Playwright Docker image against the live stack; three real pre-existing defects found and fixed in the process, see T219's Result line).
