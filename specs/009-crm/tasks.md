...
- [x] All 3 Epic 1–8 files (`users_roles/constants.py`, `main.py`, `api/v1/router.py`) confirmed additive-only, no other Epic 1–8 file touched
- [ ] All 12 CRM frontend pages functional against the live backend — built and statically verified (TypeScript/ESLint clean); live browser verification against a running dev server deliberately deferred per this phase's explicit instruction ("do NOT perform the full expensive live-verification protocol unless explicitly requested... comprehensive live verification deferred to post-Epic-9")
