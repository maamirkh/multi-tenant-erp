"""Repository for CRM feature flag overrides.
...
Spec ref: specs/009-crm/plan.md §17.2.
"""
...
class CrmFeatureFlagRepository(BaseRepository[CrmFeatureFlag]):
    """Data-access layer for ``crm_feature_flags`` table."""

    def __init__(self, db: Session) -> None:
        super().__init__(db=db, model=CrmFeatureFlag)

    def get_by_key(self, company_id: UUID, flag_key: str) -> CrmFeatureFlag | None:
        """Return the feature flag override for this company and key, or None."""
        stmt = (
            select(CrmFeatureFlag)
            .where(CrmFeatureFlag.company_id == company_id)
            .where(CrmFeatureFlag.flag_key == flag_key)
            .where(CrmFeatureFlag.is_deleted == False)  # noqa: E712
        )
