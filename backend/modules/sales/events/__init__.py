"""Sales domain events and in-process EventBus.

Provides:
  - ``SalesDomainEvent``   — base dataclass for all sales domain events
...
Spec ref: specs/007-sales-management/spec.md §33 Domain Events
Plan ref: specs/007-sales-management/plan.md Phase 0
"""
...
class SalesDomainEvent:
    """Base class for all sales domain events.
    ...
    the event (e.g. ``"Customer"`` / ``<customer_uuid>``).
    """
...
HandlerType = Callable[[SalesDomainEvent], None]


class EventBus(ABC):
    """Abstract interface for the sales domain event bus.
    ...
    @abstractmethod
    def publish(self, event: SalesDomainEvent) -> None:
...
class InProcessEventBus(EventBus):
    ...
    def subscribe(self, event_type: str, handler: HandlerType) -> None:
        ...
        logger.debug(
            "SalesEventBus: subscribed handler %s to '%s'",
    ...
    def publish(self, event: SalesDomainEvent) -> None:
        ...
        logger.debug(
            "SalesEventBus: publishing %s (aggregate=%s/%s company=%s)",
            event.event_type,
        ...
        for handler in handlers:
            try:
                ...
                logger.exception(
                    "SalesEventBus: handler %s raised an exception for event %s",
                    getattr(handler, "__qualname__", repr(handler)),
                    event.event_type,
                )
...
_sales_event_bus: EventBus = InProcessEventBus()


def get_event_bus() -> EventBus:
    """Return the active sales EventBus instance."""
    return _sales_event_bus


def set_event_bus(bus: EventBus) -> None:
    """Replace the active EventBus — used in tests and DI wiring."""
    global _sales_event_bus  # noqa: PLW0603
    _sales_event_bus = bus
