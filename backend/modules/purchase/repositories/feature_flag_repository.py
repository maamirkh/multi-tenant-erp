"""Repository for purchase feature flag overrides.

Extends ``BasePurchaseRepository`` with feature-flag-specific queries:
...
class PurchaseFeatureFlagRepository(BasePurchaseRepository[PurchaseFeatureFlag]):
    """Data-access layer for ``purchase_feature_flags`` table."""

    def __init__(self, db: Session) -> None:
        super().__init__(db=db, model=PurchaseFeatureFlag)

    def get_by_key(self, company_id: UUID, flag_key: str) -> PurchaseFeatureFlag | None:
        """Return the feature flag override for this company and key, or None."""
        stmt = (
            select(PurchaseFeatureFlag)
            .where(PurchaseFeatureFlag.company_id == company_id)
            .where(PurchaseFeatureFlag.flag_key == flag_key)
            .where(PurchaseFeatureFlag.is_deleted == False)  # noqa: E712
        )
        return self.db.execute(stmt).scalars().one_or_none()

    def get_all_for_company(self, company_id: UUID) -> list[PurchaseFeatureFlag]:
        """Return all active feature flag overrides for a company."""
        stmt = (
            select(PurchaseFeatureFlag)
            .where(PurchaseFeatureFlag.company_id == company_id)
            .where(PurchaseFeatureFlag.is_deleted == False)  # noqa: E712
            .order_by(PurchaseFeatureFlag.flag_key)
        )
