...
/**
 * AR Aging Report page — summary table by bucket with customer drill-down.
 *
...
export default function ARAgingReportPage() {
  ...
  async function load() {
    ...
    setError(null);
    try {
      const res = await getARAgingReport(companyId, asOfDate);
      setReport(res.data);
    } catch (err) {
    ...
    <div className="p-6 max-w-6xl mx-auto">
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">
            Outstanding receivables bucketed by days overdue (from due date).
          </p>
        </div>
      ...
      {loading ? (
        ...
        <div className="rounded-md border border-dashed border-gray-300 p-6 text-center text-sm text-gray-500">
          No open receivables as of {asOfDate}.
        </div>
      ) : (
        <div className="overflow-x-auto rounded-lg border border-gray-200 bg-white shadow-sm">
          <table className="w-full text-sm">
            ...
            <tbody>
              {report.rows.map((row) => (
                <tr key={row.customer_ledger_id ?? "unknown"} className="border-b border-gray-100">
                  <td className="px-4 py-2">
                    {row.customer_id ? (
                      <Link
                        href={`/${companyId}/receivables/customers/${row.customer_id}`}
